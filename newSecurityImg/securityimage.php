...
require_once($__DIR__."/../lib/no-cache.php");
//引入資料庫連結程式，並回傳狀態;
ob_start();
session_start();
...
$bgurl = rand(1, 3);
$im = $__DIR__."/images/bg".$bgurl.".png";
...
$cmd .= " -pointsize ".$size;
$cmd .= " -gravity \"".$gravity."\"";
$cmd .= " -draw \"text ".rand(0,50).",".rand(-5,5)." '".$textstr."'\"";
$cmd .= " -rotate ".$angle;
$cmd .= " ".$im." ".$__DIR__."\\images\\tmp\\".$tmpname;
exec($cmd);
...
//echo md5($textstr);
//
print fread(fopen($__DIR__.'/images/tmp/'.$tmpname, 'r'), filesize($__DIR__.'/images/tmp/'.$tmpname));
# since we've displayed the image lets go ahead and delete the actual file
exec('del '.$__DIR__.'\\images\\tmp\\'.$tmpname);
ob_end_flush();
?>