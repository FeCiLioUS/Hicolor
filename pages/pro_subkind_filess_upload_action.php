<?
//定義URL常數及變數;
define("ERRO_HEAD_URL","PRO_SUBKIND_Upload.php");
define("OK_HEAD_URL","PRO_SUBKIND_Upload.php");
define("PRVL_SN",1);
...
if(LOGIN_PRVL() == 1) {
	if($_GET['MODE'] == "DEL") {
	  ...
	  //foreach_data($uploae_mode); 
	  //查詢上傳資料表欄位;
	  $QUERY_NM = "NEWS_UPLOAD_CHECK";
	  $QUERY_SELECT_NM = "*";
	  $QUERY_TABLES_NM = "PRO_SUBKIND_PHOTO";
	  $TABLE = $QUERY_TABLES_NM;
	  $FIELD_NM = TABLE_Q($QUERY_NM,$QUERY_SELECT_NM,$QUERY_TABLES_NM);//不用改;
	  //echo $FIELD_NM."<br>";
	  //上傳資料寫入及拷貝;s
	  $SAVE_KIND = "SUBKIND";//上傳功能名稱;
	  $SAVE_ROUT = "../subkind_file/";//上傳路徑;	  
