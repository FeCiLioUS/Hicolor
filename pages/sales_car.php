...
if($CHECK_BUYED_R[0] == 0){
	...
	$TRANS_NUM = array();
	if($TRANS_R[0] == 1){		
		array_push($trans_data, $TRANS_R[1]);
	} else if ($TRANS_R[0] > 1){
	...
	foreach($trans_data as $KEY => $VALUE){
		...
		array_push($TRANS_PRICE, array("price" => $VALUE["TRANS_PRICE"]));
		array_push($TRANS_UP, array("up" => $VALUE["TRANS_UP_NUM"]));
		$t++;
	}
...
$hicolor-> assign('buyInfo', $buyInfo);
